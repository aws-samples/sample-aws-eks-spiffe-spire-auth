...
module "eks" {
  ...
  access_entries = {
    spire_tools_access = {
      principal_arn = var.root_cluster_spire_tools_iam_role_arn
      policy_associations = {
        spire_tools_policy = {
...
resource "kubectl_manifest" "spire_system_namespace" {
  yaml_body = <<YAML
apiVersion: v1
kind: Namespace
metadata:
...
YAML
  depends_on = [
    module.eks_blueprints_addons,
    kubectl_manifest.spire_system_namespace
  ]
}
...
resource "kubernetes_service_account" "spire_agent" {
  metadata {
    name      = "${local.name}-token-sa"
    namespace = local.namespace
    annotations = {
      "eks.amazonaws.com/role-arn" = aws_iam_role.eks_role.arn
    }
  }
  depends_on = [kubectl_manifest.spire_system_namespace]
}

resource "kubectl_manifest" "kubeconfigtoken_secret" {
  yaml_body = <<YAML
apiVersion: v1
kind: Secret
...
metadata:
  ...
  namespace: ${local.namespace}
  annotations:
    kubernetes.io/service-account.name: "${local.name}-token-sa"
YAML
  depends_on = [kubectl_manifest.spire_system_namespace]
}

resource "kubernetes_cluster_role" "spire_agent_token_reviewer" {
  metadata {
    name = "${local.name}-token-reviewer"
  }
  rule {
  ...
  rule {
    api_groups = [""]
    resources  = ["nodes", "pods", "configmaps", "endpoints"]
  ...
  rule {
    ...
    resource_names = ["spire-bundle-downstream", "spire-bundle-upstream"]
  }
  rule {
...
resource "kubernetes_cluster_role_binding" "spire_agent_token_reviewer_binding" {
  metadata {
    name = "${local.name}-token-reviewer-binding"
  }

  subject {
    kind      = "ServiceAccount"
    name      = kubernetes_service_account.spire_agent.metadata[0].name
    namespace = local.namespace
  }

  role_ref {
    kind     = "ClusterRole"
    name     = kubernetes_cluster_role.spire_agent_token_reviewer.metadata[0].name
